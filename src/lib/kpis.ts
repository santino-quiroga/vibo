...
/** Una franja horaria con su precio, en minutos del día: [desdeMin, hastaMin). */
export type FranjaPrecio = {
  desdeMin: number;
  hastaMin: number;
  precio: number;
};
...
export type CanchaConfig = {
  numero: number;
  /** Precio base: se cobra en las horas que ninguna franja cubre. */
  precio: number;
  /** Tarifas por franja horaria; vacío = un solo precio a toda hora. */
 ...
 * Se busca la primera franja que contenga la hora —rango [desde, hasta)— y, si
 * ninguna la cubre (o el turno no tiene hora), cae al precio base de la cancha.
 * Las franjas se validan sin solaparse al guardar, así "la primera" es la única.
 */
export function precioEnFranja(
  cancha: CanchaConfig,
  horaInicioMin: number | null,
): number {
  if (horaInicioMin !== null) {
    const franja = cancha.franjas.find(
      (f) => horaInicioMin >= f.desdeMin && horaInicioMin < f.hastaMin,
    );
    if (franja) return franja.precio;
  }
...
export type Ingresos = {
   ...
   * unitario sólo cuando la cancha cobra igual a toda hora; con franjas distintas
 ...
 * (`precioEnFranja`), porque el mismo court cobra distinto de día y de noche.
 */
export function ingresosEstimados(
  ...
  for (const reserva of turnosConfirmados(reservas)) {
    ...
    a.turnos += 1;
    a.subtotal += precioEnFranja(cancha, reserva.horaInicioMin);
    acum.set(numero, a);
  }

  const porCancha = [...acum.entries()]
    .map(([numero, { turnos, subtotal }]) => {
      // El unitario sólo tiene sentido sin franjas: con franjas, "turnos ×
      // precio" dejaría de dar el subtotal, así que se muestra sin él.
      const cancha = porNumero.get(numero);
      const precio = cancha && cancha.franjas.length === 0 ? cancha.precio : null;
